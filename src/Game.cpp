...
void Game::Init(GLFWwindow* window)
{
    ...
    camera = new Camera(glm::vec3(0.0f, 1.0f, 0.0f), speed);
    modelMeshes = GltfLoader::LoadModel("assets/models/Base.gltf", glm::vec3(16.0f, 1.0f, 16.0f));
    ...
    float cubeSize = 1.0f;
    int chunkSize = 16;
    Chunk chunk(chunkSize, cubeSize, "assets/textures/grass_block.png");
...
void Game::handleInput(GLFWwindow* window, float deltaTime)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_C) == GLFW_PRESS && now - lastC > 0.5)
    {
        locked = !locked;
    ...
    if (glfwGetKey(window, GLFW_KEY_B) == GLFW_PRESS && now - lastB > 0.5)
    {
        wf = !wf;
...
void Game::Render()
{
    ...
    ImGui::Text("XYZ: (%d, %d, %d)", (int) std::round(camera->Position.x),
                (int) std::round(camera->Position.y), (int) std::round(camera->Position.z));
...
void Game::handleResize(GLuint nwidth, GLuint nheight)
{
    renderer->handleResize(nwidth, nheight);
}
