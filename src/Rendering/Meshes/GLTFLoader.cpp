...
void ProcessNode(const tinygltf::Model &model, int nodeIndex,
                 const glm::mat4 &parentTransform, std::vector<Mesh> &meshes) {
  const auto &node = model.nodes[nodeIndex];
  glm::mat4 transform = parentTransform;

  if (!node.matrix.empty()) {
    transform *=
        glm::make_mat4(reinterpret_cast<const float *>(node.matrix.data()));
  } else {
  ...
  if (!node.scale.empty()) {
    ...
    std::cout << "Scale: " << scaleVec.x << ", " << scaleVec.y << ", "
              << scaleVec.z << std::endl;
  ...
  if (node.mesh >= 0) {
    ...
    for (const auto &prim : mesh.primitives) {
      ...
      glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                   idxAccessor.count * sizeof(unsigned int), indices,
                   GL_STATIC_DRAW);
